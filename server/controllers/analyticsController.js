 ...
 * 4. Unknown
 *
 * Numeric-only values are rejected because
 * they are usually UTM IDs / campaign IDs,
 * not real source names.
 */
function dashboardSourceExpression() {
  return {
    $let: {
      vars: {
        utmSourceValue: {
...
// GET /api/analytics/drop-reasons
async function dropReasons(
  req,
  res,
  next
) {
